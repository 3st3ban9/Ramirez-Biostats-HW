...
covid_counts<-covid|>
  ...
  summarise("count"=length(c(Vaccination_status,Survival)))
```
...
```{r}
covidmat<-xtabs(count~Vaccination_status+Survival, data=covid_counts)
```

```{r}
covidchi<-chisq.test(covidmat)
names(covidchi)
covidchi$p.value
```
...
With a p-value so low (\<.05), the results are significant. It is hard to disagree that there is a significant association between vaccination status and ER survival.
