...
```{r}
testremove<-df[complete.cases(df[,2]),]
```
...
```{r}
rm(df1, df_remove_All, df_remove_NA_Mass, df_remove_NA_growth, testremove)
```
...
Our response variable type is: Growth rate = continuous
...
Maybe there is a significant effect. Level 1 and 2 may be different from control, unsure if those two will differ from each other.

#### Create a model:

```{r}
growmod<-lm(Growth_rate_mm~Treatment, data=df_All_NA)
```
